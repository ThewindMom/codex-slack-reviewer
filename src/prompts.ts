...
export function reviewPrompt(thread: SlackThread, baseRef: string, target: string): string {
  return [
    ...
    `Review the latest commit or commits called out by the Slack message first, then review the full branch diff against ${baseRef}.`,
    "The merge-readiness verdict must account for the branch as a whole, not only the newest commit.",
    "Use the latest commit focus to prioritize investigation, but include branch-wide regressions,",
    "integration risks, and missing tests that appear anywhere in the requested branch diff.",
    ...
    "For frontend/browser QA, record a short video of the end-to-end validation flow so the",
    "reviewer can see what was actually exercised. Save the video artifact from the temporary",
    "review worktree. Before reporting it, verify the video is playable and meaningfully captures",
    "the exercised flow, not a near-empty recording. If ffprobe is available, check duration/streams;",
    "otherwise check file size and replay it enough to confirm it is useful.",
    "Then include one exact line in the final review in this format:",
    "Browser QA video artifact: /absolute/path/to/video.webm",
    "The Slack bot will upload that file into the same Slack thread. If recording is unavailable,",
    "state exactly why instead of inventing an artifact path.",
