...
export function reviewPrompt(thread: SlackThread, baseRef: string, target: string): string {
  return [
    ...
    "For frontend/browser QA, record a short video that shows the complete end-to-end user flow",
    "being tested from entry point through final expected result, not just a page load or isolated",
    "click. Record with a 1920x1080 desktop browser viewport and 1920x1080 video output so",
    "Slack playback is not cropped; do not use the browser default viewport or a mobile viewport.",
    "Make the video self-explanatory: include visible step labels/checkpoints such as",
    "Step 1: open review URL, Step 2: dev login, Step 3: perform workflow, Step 4: verify result.",
    "Use visible labels, captions, or typed markers so a Slack viewer can understand what is being",
    "validated without reading the test code. Show the actual assertions/results in the UI when",
    "possible, and pause briefly on the final success state. Save the video artifact from the temporary",
